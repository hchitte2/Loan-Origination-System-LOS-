...
Status: not started · 2–3 sessions

**Tasks:** `server/storage.ts` · `/api/upload` with `handleUpload` (dual auth, types, 10 MB, prefix, caps) · `UploadZone` + `registerDocument` / `registerPublicDocument` · `/api/files/[documentId]` · Needs list tab: documents under their condition, accept, reject with reason, download, clear (with "Clear this condition?" prompt after accepting the last document), waive with reason · `/u/[token]` from artboard 5 with all drawn states, e-consent stored in the `document.uploaded` detail · `/queue` with KPI tiles and the review list · `limits.ts` enforced and unit-tested · specimens uploaded once by `pnpm seed:files` · extend `a11y.spec.ts` with `/queue` and `/u/[token]`.
