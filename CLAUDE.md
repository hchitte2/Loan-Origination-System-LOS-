...
## Stack facts
Next.js 16 App Router (Server Components + Server Actions, `proxy.ts`), React 19, TypeScript 5.9, Tailwind 4 + shadcn/ui (charts are token-driven CSS bars, no chart library), next-themes, Drizzle + Neon Postgres (`neon-serverless` on Vercel, `pg` locally), Better Auth 1.7 with the admin plugin (impersonation), Vercel Blob private store, zod 4, pnpm, Biome, vitest, Playwright + axe. Hosting: Vercel Hobby + Neon Free + Blob. Local DB: the Neon `dev` branch. Not used on purpose: TanStack Query, RLS, websockets, LLM calls, dnd-kit, Docker.
